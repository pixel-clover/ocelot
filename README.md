...
console warning and never stops emulation, because a still picture is usually not a
fault: title screens, pause menus, and anything waiting on input hold a frame
indefinitely and will trip it too.

A crash is reported separately and on screen, so a freeze that shows no error message
is a different fault from one that does. Mentioning which you saw is useful.
