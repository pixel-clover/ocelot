...
spec = do
    ...
    describe "IRQ edge detection" $ do
        ...
        it "row-select change exposes a held button as a falling edge" $ do
            -- Press Up first while the direction row is deselected, so the press itself does not
            -- fire an IRQ. Then switch the row selector to expose direction; the low nibble flips bit 2
            -- from 1 to 0 and that should latch a joypad-IRQ. Without this, games that use
            -- 'select row + HALT' to wait for input can miss buttons held across the row change.
            jp <- initial
        ...
        it "press in shared column with both rows selected and other-row button held does not fire" $ do
            -- Regression: with both rows selected (sel=0x00), bit 0 of the low nibble is the
            -- AND of A (action row) and Right (direction row). If Right is already held (bit 0 = 0)
            -- and the user then presses A, the bit stays at 0; no falling edge.
            -- The previous implementation always fired an IRQ on a fresh selected-row press,
            -- missing this AND interaction. SameBoy 'GB_update_joyp' computes the OR across rows
            -- and edge-detects the merged byte.
            jp <- initial
