// Differential trace driver: runs a ROM in SameBoy with the execution
// callback set to a per-instruction state dumper, emitting one line per
// instruction in the same format as Ocelot's tools/ocelot-trace tool.
// Pipe both into diff to find the first divergence.
//
...
static const unsigned char boot_stub[0x100] = {
    ...
    // post-boot F value; without this the F register starts at 0x00 and
    // any AF-comparing diff against Ocelot drifts on instruction 1.
    [0x00] = 0xAF,                    // XOR A
    [0x01 ... 0xD8] = 0x00,           // NOP padding

    // LCDC = 0x91 (LCD on, BG on, BG tile data 0x8000, BG tilemap 0x9800)
    [0xD9] = 0x3E, [0xDA] = 0x91,     // LD A, 0x91
    [0xDB] = 0xE0, [0xDC] = 0x40,     // LDH (FF40), A
    // BGP = 0xFC
    [0xDD] = 0x3E, [0xDE] = 0xFC,     // LD A, 0xFC
    [0xDF] = 0xE0, [0xE0] = 0x47,     // LDH (FF47), A
    // OBP0 = 0xFF, OBP1 = 0xFF (A reused)
    [0xE1] = 0x3E, [0xE2] = 0xFF,     // LD A, 0xFF
    [0xE3] = 0xE0, [0xE4] = 0x48,     // LDH (FF48), A
    [0xE5] = 0xE0, [0xE6] = 0x49,     // LDH (FF49), A
    // NR50 = 0x77
    [0xE7] = 0x3E, [0xE8] = 0x77,     // LD A, 0x77
    [0xE9] = 0xE0, [0xEA] = 0x24,     // LDH (FF24), A
    // NR51 = 0xF3
    [0xEB] = 0x3E, [0xEC] = 0xF3,     // LD A, 0xF3
    [0xED] = 0xE0, [0xEE] = 0x25,     // LDH (FF25), A
    // NR52 = 0x80 (APU on, no channels enabled)
    [0xEF] = 0x3E, [0xF0] = 0x80,     // LD A, 0x80
    [0xF1] = 0xE0, [0xF2] = 0x26,     // LDH (FF26), A
    // CPU register pairs (BC defaults to 0, H to 0).
    [0xF3] = 0x16, [0xF4] = 0xFF,     // LD D, 0xFF
    [0xF5] = 0x1E, [0xF6] = 0x56,     // LD E, 0x56
    [0xF7] = 0x2E, [0xF8] = 0x0D,     // LD L, 0x0D
    [0xF9] = 0x31, [0xFA] = 0xFE, [0xFB] = 0xFF, // LD SP, 0xFFFE
    [0xFC] = 0x3E, [0xFD] = 0x11,     // LD A, 0x11
    [0xFE] = 0xE0, [0xFF] = 0x50,     // LDH (FF50), A   ; unmap, hand off
};

// Required no-op callbacks. SameBoy crashes mid-frame if some of these
// are NULL (specifically rgb_encode and vblank are exercised by the PPU
// once it leaves the LCD-off state).
static char *async_input_callback(GB_gameboy_t *unused) { (void)unused; return NULL; }
...
static void on_log(GB_gameboy_t *unused, const char *msg, GB_log_attributes_t attrs)
{
    (void)unused;
    (void)msg;
    (void)attrs;
...
{
    (void)opcode;
    if (instructions_remaining == 0) return;
    // Skip the boot-stub instructions; only emit trace lines once we
    // reach the cart's entry point at 0x100, so the trace lines up
    // 1:1 with Ocelot's starting from cgbPostBoot.
    if (!reached_cart) {
        if (pc != 0x0100) return;
        reached_cart = true;
    }
