...
- `app-web/Main.hs`: WASM executable entry point. Exposes the emulator to JavaScript via exported WASM functions; compiled with the GHC WASM
  ...
  JavaScript.** The wasm linker exports only the names listed as `-optl-Wl,--export=<name>` under the `wasm-reactor` flag in `package.yaml`, so a new
  export needs an entry there too. Forgetting it fails silently: the code compiles, the deploy succeeds, and the symbol is simply absent from
  `ocelot.wasm`. Check a built artifact with `strings -n 8 dist/web/ocelot.wasm | grep '^ocelot_'`.
